...
### 절대 경로와 상대 경로의 차이
  - 절대 경로: 시스템의 최상위 디렉토리(루트, `/`)부터 시작하여 현재 위치까지의 전체 경로를 의미함. (예: `/Users/username/Desktop/folder`)
  - 상대 경로: 현재 작업 중인 위치를 기준으로 목적지의 경로를 나타냄. 현재 위치는 `./`, 상위 위치는 `../`로 표현함.

### 파일 권한 755, 644의 규칙과 해석
  - 권한은 읽기(r=4), 쓰기(w=2), 실행(x=1)의 합으로 계산되며 소유자/그룹/기타 사용자 순으로 나열.
  - `644`: 소유자는 읽기/쓰기(6), 나머지는 읽기(4)만 가능함을 뜻함. (일반 파일의 기본 권한)
  - `755`: 소유자는 읽기/쓰기/실행(7), 나머지는 읽기/실행(5)이 가능함을 뜻함. (디렉토리나 실행 파일에 주로 부여)

### 포트 매핑이 필요한 이유
  - Docker 컨테이너는 호스트 컴퓨터(내 PC)와 네트워크가 완전히 격리되어 있음. 따라서 컨테이너 내부에서 웹 서버가 동작하고 있더라도 외부에서 접근할 수 없기 때문에, 호스트의 특정 포트(예:8080)로 들어오는 접속을 컨테이너 내부의 지정된 포트(예:80)로 연결해주는 통로 역할이 필요함.

### Git과 GitHub의 차이
  - `Git`은 내 컴퓨터(로컬)에서 코드의 버전 이력을 기록하고 관리하는 소프트웨어.
  - `GitHub`는 Git으로 관리하는 저장소를 인터넷상에 업로드하여 백업하고, 여러 사람과 협업할 수 있도록 돕는 클라우드 기반의 플랫폼.
...
## 1. 제출 저장소
- Github Repository는 요구사항목록에 맞춰서 Public으로 설정하였다.
- 레포지토리 주소
```
...
- `git config --global` 명령어를 사용하여 `user.name`, `user.email`으로 이름과 이메일 주소를 설정하고, `init.defaultBranch main`을 사용하여 과제의 요구사항인 브렌치 이름 `main`으로 설정하기를 충족시켰다.
- `git config --list`를 사용하여 잘 되었는지 전체 목록을 출력하게 하였다.
...
### 11. 보안 및 개인정보 보호
- 기술 문서에 적힌 민감 개인 정보를 마스킹 처리 하였다. 민감 정보는 다음과 같이 변경하였다.
```
"깃허브 닉네임", "깃허브 이메일", "개인정보 마스킹", "암호화 번호"
```
...
- 문제: 바인드 마운트(8. 포트 매핑 및 접속 증거) 실습 중 컨테이너는 정상 실행되었으나, 브라우저로 `localhost:8081` 접속 시 `403 Forbidden` 에러가 발생함. 터미널에서는 `dquote>` 기호가 뜨며 입력이 멈추는 현상이 있었음.
- 원인: 터미널에서 echo 명령어로 파일을 생성할 때, 따옴표의 짝이 맞지 않았고 결과를 파일로 넘겨주는 리다이렉션 기호(`>`)가 누락됨. 이로 인해 `bind-html` 폴더 내부에 `index.html` 파일이 생성되지 않아서 Nginx 웹 서버가 띄워줄 페이지를 찾지 못해 403 접근 금지 에러를 띄운 것으로 추정함.
